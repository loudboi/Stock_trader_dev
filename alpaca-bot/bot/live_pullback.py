...
class PullbackLiveTrader:
    ...
    def _wait_pending(self, name, inst, seconds=ORDER_CONFIRM_SECONDS):
        ...
        if name in self.state["pending"]:
            self.notify(f"ORDER UNCERTAIN: {name} accepted but not broker-confirmed; blocking symbol.")
            return False
        return True
    ...
    def _confirm_pending(self, name, inst):
        ...
        if pending["type"] == "buy":
            ...
            self.notify(f"BUY {name} tranche {pos['tranches']}/{len(self.params.tranches)} "
                        f"qty={pending['requested_qty']} @~{fill_px:.4f} ({pending['how']})" +
    ...
    def _process(self, name):
        ...
        if pos and broker is None:
            self._finalize_external_close(name, inst, pos); return
        if broker and pos is None:
            pos = self._handle_untracked_broker_long(name, inst, broker)
            self.state["intents"].pop(name, None); self.save_state()
        elif broker and pos:
