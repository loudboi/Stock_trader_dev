...
# after_tax_buy_hold
# --------------------------------------------------------------------------- #
def test_after_tax_buy_hold_taxed_once_at_the_end_if_under_exempt_period():
    idx = pd.date_range("2020-01-01", periods=400, freq="D")   # ~1 year, well under 15y
    r = pd.Series(0.0, index=idx)
    r.iloc[-1] = 0.0
    # Build a return series that nets +50% total.
    growth = 1.5
    r = pd.Series(growth ** (1 / len(idx)) - 1, index=idx)
    eq = tx.after_tax_buy_hold(r, tax_rate=0.25, exempt_days=15 * 365 + 1, initial=100_000.0)
...
def test_exposure_based_matches_raw_growth_for_one_uninterrupted_holding():
    ...
    eq = tx.after_tax_exposure_based(close, held, tax_rate=0.25, initial=100_000.0)
...
def test_exposure_based_multiple_trades_each_taxed_independently():
    ...
    no_tax_eq = tx.after_tax_exposure_based(close, held, tax_rate=0.0, initial=100_000.0)
